...
{
    private const string PluginName = "InventoryReporter2";
    ...
    private void DrawRetainerListButton()
    {
        try
        {
            ...
            ImGui.SameLine();
            if (ImGuiComponents.IconButton("InventoryReporterRefresh", FontAwesomeIcon.BookOpen))
                StartFullRefresh();
        ...
        {
            log.Error(ex, "[InventoryReporter2] Error drawing AutoRetainer refresh button");
        }
    }

    private void OnRetainerAdditionalTaskReceived(string retainerName)
    {
        if (skipPostprocessUntilManualStart || !isRefreshing)
            return;
    ...
    private void FailRefresh(string message, Exception ex)
    {
        ...
        log.Error(ex, $"[InventoryReporter2] {message}");
        isRefreshing = false;
        isStartQueued = false;
        ...
        {
            log.Warning(finishEx, "[InventoryReporter2] Unable to release AutoRetainer postprocess lock after refresh failure");
        }
    }
